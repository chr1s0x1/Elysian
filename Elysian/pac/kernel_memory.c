...
kernel_vm_remap(uint64_t address, size_t size) {
	...
	if (kr != KERN_SUCCESS) {
		...
		ERROR("Could not remap 0x%016llx [0x%zx]", address, size);
		return NULL;
	}
...
void
kernel_vm_unmap(void *address, size_t size) {
	kern_return_t kr = mach_vm_deallocate(mach_task_self(), (mach_vm_address_t) address, size);
	if (kr != KERN_SUCCESS) {
		WARNING("%s returned %d: %s", "mach_vm_deallacate", kr, mach_error_string(kr));
		WARNING("Could not unmap %p [0x%zx]", address, size);
	}
}
...
kernel_read(uint64_t address, void *data, size_t size) {
	...
	while (size > 0) {
		...
		kern_return_t kr = mach_vm_read_overwrite(kernel_task_port, address,
				read_size, (mach_vm_address_t) read_data, &size_out);
		if (kr != KERN_SUCCESS) {
			ERROR("%s returned %d: %s", "mach_vm_read_overwrite", kr,
					mach_error_string(kr));
			ERROR("Could not %s address 0x%016llx", "read", address);
			return false;
		}
		if (size_out != read_size) {
			ERROR("Partial read of address 0x%016llx: %llu of %zu bytes",
					address, size_out, read_size);
			return false;
		}
		address += read_size;
		read_data += read_size;
